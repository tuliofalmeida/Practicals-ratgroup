...
def SetGLMsParams(Nav,Spk):   
    ...
    USAGE:
        Nav = LoaddataNav(loadparams)
        Spk = LoaddataSpk(loadparams, Nav);
        glmsparams = SetGLMsParams(Nav, Skp)
    ...
    glmsparams['subset']['Condition_op'] = np.isin

    glmsparams['subset']['XDir'] = [1]
    glmsparams['subset']['XDir_op'] = np.isin

    glmsparams['subset']['laptype'] = [-1,0,1]
    glmsparams['subset']['laptype_op'] = np.isin
                
    glmsparams['subset']['Spd'] = 2.5
    glmsparams['subset']['Spd_op'] = np.greater_equal    

    # Subset of cells for which GLMs will be computed        
    glmsparams['cellidx'] = np.ones(Spk['spikeTrain'].shape[1]).astype(bool)

    # Sampling rate of the data
    glmsparams['sampleRate'] = 1/np.nanmean(np.diff(Nav['sampleTimes']))

    # Scaling factor on the response data (default is 1 / samplingRate so that
    # spiking data are returned in spike / s)           
    glmsparams['scalingFactor'] = 1 / glmsparams['sampleRate']

    # Name of the field of Nav that'll be used as the first predictor. Default is Xpos and the
    # Name of the field of Nav that'll be used as the second predictor. Default is Spd
    glmsparams['variablename'] = ['Xpos','Spd']

    # Size of the gaussian window for smoothing tuning curves along the first predictor (in bins).
    glmsparams['smthNbins'] = [1,1]

    # Edges of bins used to discretize the first predictor.
    glmsparams['binedges'] = [np.arange(0,104,4),
                            np.concatenate((np.arange(2.5,50,5), np.array([np.inf])))]

    # Occupancy threshold above which predictors are included in the GLM estimate
    glmsparams['occ_th'] = 0
    ...
    # Type of regularization in glmnet: 1 for lasso, 0 for ridge, elsticnet for everything in between
    glmsparams['alpha'] = 1

    # Maximal number of passes over the data before reaching convergence
    glmsparams['maxit'] = 10**3

    # Convergence threshold for coordinate descent, expressed as a fraction of the null deviance.
    glmsparams['thresh'] = 10**-3

    # %P-value threshold for considering a predictor as significantly contributing to the cell response.
